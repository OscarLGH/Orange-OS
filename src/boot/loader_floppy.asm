...
LABEL_DESC_VIDEO:   Descriptor 0B8000h,	0fffffh, DA_DRW|DA_DPL3|DA_LIMIT_4K ; 显存首地址
...
LABEL_PM_START:
	...
	mov eax,[ScreenX]
	mov [BOOT_PARAM_ADDR + 16],eax
	mov eax,[ScreenY]
	mov [BOOT_PARAM_ADDR + 20],eax
	;***************************************************************
...
.1:				  ;               LengthHigh,Type
	...
	add	eax, [dwLengthLow];
	cmp	eax, [ddMemSize]  ;    if(BaseAddrLow + LengthLow > MemSize)
	jb	.2		  ;
	mov	[ddMemSize], eax  ;    MemSize = BaseAddrLow + LengthLow;
.2:				  ;  }
	loop	.loop		  ;}
				  ;
	call	DispReturn	  ;printf("\n");
