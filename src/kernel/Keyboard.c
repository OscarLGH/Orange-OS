...
PUBLIC	void KeyboardHandler(int irq)
{
	...
	PrintIntPos(scan_code,1,80*16*2,0x3c);
...
PUBLIC void ReadKeyboard(TTY * p_tty)
{
	u8 scan_code;
	char output[2];
	...
	if(kb_in.count > 0)
	{
		...
		{
			int i;
			u8 pausebrk_scode[] = {0xe1,0xd1,0x45,0xe1,0x9d,0xc5};
			int is_pausebreak = 1;
			for(i = 1; i < 6;i++)
...
PUBLIC void SetLEDs()
{
	...
	while(In_Byte(0x64)&0x02);		//等待8042输入缓冲区为空
	Out_Byte(0x60,0xED);
	/*
...
PUBLIC void FlashLEDs()
{
	...
	while(1)
	{
		...
		num_lock = 0;
		scroll_lock = 0;
		leds = (caps_lock << 2)|(num_lock << 1)|scroll_lock;
		while(In_Byte(0x64)&0x02);		//等待8042输入缓冲区为空
		Out_Byte(0x60,0xED);
		while(In_Byte(0x60)!= 0xFA);	//等待响应
		while(In_Byte(0x64)&0x02);		//等待8042输入缓冲区为空
